...
fun Canvas.line(x1: Int, y1: Int, x2: Int, y2: Int) : Either<AppError, Canvas> {
    return when {
        !isInWriteSpace(x1, y1) || !isInWriteSpace(x2, y2) -> Either.Left(CanvasDrawingError("coordinate out of bounds"))
        !isHorizontal(x1, x2) && !isVertical(y1, y1) -> Either.Left(CanvasDrawingError("only vertical/horizontal lines supported"))
        else -> {
            (x1..x2).forEach {x ->
                (y1..y2).forEach { y ->
                    this[y][x] = 'x'
